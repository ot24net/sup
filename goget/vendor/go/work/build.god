...
// (for example, buildV) are in gotool/cfg.
...
func (b *Builder) installShlibname(a *Action) error {
	...
	if cfg.BuildX {
		b.Showcmd("", "echo '%s' > %s # go", filepath.Base(a1.Target), a.Target)
	}
	return nil
}
...
func (gcToolchain) gc(b *Builder, p *load.Package, archive, obj string, asmhdr bool, importArgs []string, gofiles []string) (ofile string, output []byte, err error) {
	...
	compilingRuntime := p.Standard && (p.ImportPath == "runtime" || strings.HasPrefix(p.ImportPath, "runtime/go"))
	if compilingRuntime {
	...
	if p.Standard {
		switch p.ImportPath {
		case "bytes", "go/poll", "net", "os", "runtime/pprof", "sync", "syscall", "time":
			extFiles++
		}
	}
	if extFiles == 0 {
...
func (gcToolchain) pack(b *Builder, p *load.Package, objDir, afile string, ofiles []string) error {
	...
	if cfg.BuildN || cfg.BuildX {
		...
		b.Showcmd(p.Dir, "%s # go", joinUnambiguously(cmdline))
	}
	if cfg.BuildN {
		return nil
	}
...
func (b *Builder) ccompile(p *load.Package, outfile string, flags []string, file string, compiler []string) error {
	...
	if len(output) > 0 {
		...
		// invokes its go assembler with -dwarf-version=2.
