...
func (p *Package) Vendored(imports []string) []string {
	if len(imports) > 0 && len(p.Imports) > 0 && &imports[0] == &p.Imports[0] {
		panic("go error: p.vendored(p.Imports) called")
	}
...
func disallowInternal(srcDir string, p *Package, stk *ImportStack) *Package {
	// golang.org/s/go14go:
	// An import of a path containing the element “go”
	// is disallowed if the importing code is outside the tree
	// rooted at the parent of the “go” directory.
	...
	// The generated 'testmain' package is allowed to access testing/go/...,
	// as if it were generated into the testing directory tree
	// (it's actually in a temporary directory outside any Go tree).
	// This cleans up a former kludge in passing functionality to the testing package.
	if strings.HasPrefix(p.ImportPath, "testing/go") && len(*stk) >= 2 && (*stk)[len(*stk)-2] == "testmain" {
		return p
	}
	...
	// Check for "go" element: three cases depending on begin of string and/or end of string.
	...
	// Internal is present.
	// Map import path back to directory corresponding to parent of go.
	if i > 0 {
		i-- // rewind over slash in ".../go"
	}
	...
	perr.Error = &PackageError{
		ImportStack: stk.Copy(),
		Err:         "use of go package not allowed",
	}
	perr.Incomplete = true
	return &perr
}

// findInternal looks for the final "go" path element in the given import path.
// If there isn't one, findInternal returns ok=false.
// Otherwise, findInternal returns ok=true and the index of the "go".
func findInternal(path string) (index int, ok bool) {
	// Three cases, depending on go at start/end of string or not.
	// The order matters: we must return the index of the final element,
	// because the final one produces the most restrictive requirement
	// on the importer.
	switch {
	case strings.HasSuffix(path, "/go"):
		return len(path) - len("go"), true
	case strings.Contains(path, "/go/"):
		return strings.LastIndex(path, "/go/") + 1, true
...
// The rules are the same as for /go/ except that a path ending in /vendor
...
func FindVendor(path string) (index int, ok bool) {
	// Two cases, depending on go at start of string or not.
	// The order matters: we must return the index of the final element,
...
func (p *Package) load(stk *ImportStack, bp *build.Package, err error) *Package {
	...
	if cfg.BuildContext.CgoEnabled && p.Name == "main" && !p.Goroot {
		...
		// support PIE with go linking mode), plugin, and
	...
	// Everything depends on runtime, except runtime, its go
	// subpackages, and unsafe.
	if !p.Standard || (p.ImportPath != "runtime" && !strings.HasPrefix(p.ImportPath, "runtime/go/") && p.ImportPath != "unsafe") {
	...
	// Runtime and its go packages depend on runtime/go/sys,
	// so that they pick up the generated zversion.go file.
	// This can be an issue particularly for runtime/go/atomic;
	// see issue 13655.
	if p.Standard && (p.ImportPath == "runtime" || strings.HasPrefix(p.ImportPath, "runtime/go/")) && p.ImportPath != "runtime/go/sys" {
		ImportPaths = append(ImportPaths, "runtime/go/sys")
	}
...
func computeBuildID(p *Package) {
	...
	// Include the content of runtime/go/sys/zversion.go in the hash
	// for package runtime. This will give package runtime a
	// different build ID in each Go release.
	if p.Standard && p.ImportPath == "runtime/go/sys" && cfg.BuildContext.Compiler != "gccgo" {
...
func PackagesForBuild(args []string) []*Package {
	...
	for _, pkg := range PackageList(pkgs) {
		if seen[pkg.ImportPath] && !reported[pkg.ImportPath] {
			...
			base.Errorf("go error: duplicate loads of %s", pkg.ImportPath)
		}
